...
def main():
    ...
    for proces in processen:
        ...
        for fraction in fractions:
            rat_dict = get_test_data(fraction, mouse_gaf, blast_input, go_tree)
...
def get_test_data(fraction, mouse_gaf, blast_input, tree):
    sample = Splitter(fraction, mouse_gaf).splitter()
    true_set = parse_true_annotation(sample)
    rat_dict = {}
    for ids in blast_input:
        if ids in true_set:
            rat_dict[ids] = fix_go(true_set[ids], tree)
    return rat_dict
